...
if (window.jQuery)(function ($) {
	...
	$.fn.MultiFile = function (options) {
		...
		this
			...
			.each(function () {
				...
				// add from namtwon 
				MultiFile.oldList = [];
				MultiFile.deletedList = [];
				
				//===
				// add from nam tae won 2015.12.23
				MultiFile.inputOptions = $.extend(true,{},o);
				//#########################################
				...
				MultiFile.addSlave = function (slave, slave_count) {
					...
					slave.change(function (a, b, c) {
						...
						var prevsListLength = MultiFile.oldList.length - MultiFile.deletedList.length;
						if (MultiFile.max>0 && prevsListLength + prevs.length + files.length > MultiFile.max) {
				...
				MultiFile.addToList = function (slave, slave_count, files) {
						...
						var v = String(file.name || '' ),
								...
								label =	$(
										(
												...
												"<span style='padding-left:10px'>(" + sl(file.size || 0) + ")</span>" +
					...
					//$.each(files, function (i, file) {
						...
						MultiFile.list.append(
								...
								r.append(names, ' ',b )//modify : nam tae won  r.append(b, ' ', names)
						);
	...
	*/
	$.extend($.fn.MultiFile, {
		...
		*/
		...
		toDeletedList : function(){
			var mf = this.MultiFile('data');
			if(!mf) return !console.log('MultiFile plugin not initialized');
			return mf.deletedList;
		},
		toRemainList : function(){
			var mf = this.MultiFile('data');
			if(!mf) return !console.log('MultiFile plugin not initialized');
			var rtn = [];
			for(var i=0; i < mf.oldList.length ; i++){
				var fileId = mf.oldList[i]['fileId'];
				if(!fileId) continue;
				var files = $.grep(mf.deletedList,function(element,index){
					return element.fildId == fileId;
				});
				if(!files || !files.length || files.length ==0){
					rtn.push(mf.oldList[i]);
				}
			}
			return rtn;
		},
		/**
		 * attach file from database : default property 
		 * {
		 * 	fileName:''
		 * 	
		 * }
		 */
		addList : function(options,attachOption){
			var defaultOption = {
				fileName :'',
				fileExtension :'',
				fileSize : 0,
		...
		getFileCss : function(fileExt){
			var sExt = (fileExt||'').toLowerCase();
			var sCssName = "";
			if(/(doc|docx)/.test(sExt)){sCssName = "docx";}
			else if(/(xls|xlsx)/.test(sExt)){sCssName = "xlsx";}
			else if(/(ppt|pptx)/.test(sExt)){sCssName = "pptx";}
			else if(/(bmp|jpg|jpeg|png)/.test(sExt)){sCssName = "jpg";}
			else if(/(gif)/.test(sExt)){sCssName = "gif";}
			else if(/(hwp)/.test(sExt)){sCssName = "hwp";}
			else if(/(pdf)/.test(sExt)){sCssName = "pdf";}
			else if(/(rar|zip)/.test(sExt)){sCssName = "zip";}
			else if(/(mp3)/.test(sExt)){sCssName = "mp3";}
			else if(/(mp4|wav|wmv|avi)/.test(sExt)){sCssName = "avi";}
			else {sCssName = "other";}
			return sCssName;
		}
	});
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		STRING: {
			remove: 'x',
			denied: '확장자 $ext 인 파일은 업로드 할 수 없습니다.',
			file: '$file',
			selected: 'File selected: $file',
			duplicate: '이미 선택된 파일입니다.:\n$file',
			toomuch: '총 $size 까지만 업로드가 가능합니다.',
			toomany: '총 $max 개만 업로드 가능합니다.',
			toobig: '$file : $size 까지만 업로드 가능합니다.'
		},
